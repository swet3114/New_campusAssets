...
export default function ManageUsers() {
  ...
  async function fetchUsers() {
    ...
    try {
      const res = await fetch(
        `${API}/api/users${roleFilter ? `?role=${roleFilter}` : ""}`,
        { credentials: "include" }
      );
      ...
      if (data.success) {
        setUsers(data.users);
        const countObj = {};
        (data.counts || []).forEach(c => { countObj[c._id] = c.count; });
        setCounts(countObj);
      } else {
  ...
  // Style objects for beautiful action buttons
  const actionBtn = {
    padding: "6px 16px",
    border: "none",
    borderRadius: "6px",
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 32 }}>
      ...
      <div style={{ display: "flex", gap: 24, marginBottom: 20 }}>
        ...
        <button
          style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #e5e7eb", background: !roleFilter ? "#eff6ff" : "#fff" }}
      ...
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, background: "#fff" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
              ...
              <tr key={u._id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                ...
                <td style={{ padding: 9, textAlign: "left" }}>
                  ...
                  {resetPwId !== u._id ? (
                    <button
                      style={resetBtn}
                      onClick={() => { setResetPwId(u._id); setNewPw(""); }}
                    ...
                    <span>
                      ...
                      <button
                        style={saveBtn}
                        onClick={() => resetPassword(u._id, newPw)}
                      >
                        Save
                      </button>
                      <button
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={4} style={{ textAlign: "center", padding: 18, color: "#9ca3af" }}>No users found.</td>
              </tr>
            )}
          </tbody>
        </table>
